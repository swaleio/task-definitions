...
description: Downloads a model, dataset, or space snapshot from the Hugging Face Hub into the workspace, with optional revision, glob filtering, and token auth for gated/private repos.
inputs:
  repo:
...
outputs:
  path:
    description: The local directory the content was downloaded to, under the workspace.
exec:
...
Downloads `repo` from the Hugging Face Hub into `/mnt/workspace/<dest>` and emits
the local `path` for downstream tasks. The wrapper exports `HF_TOKEN` from the
...
| `dest` | no | `model` | Destination directory under the workspace. |
...
| Output | Description |
|--------|-------------|
| `path` | The local directory the content was downloaded to (e.g. `/mnt/workspace/model`). |
