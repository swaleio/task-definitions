...
inputs:
  ...
  expected_status:
    ...
    default: ""
  response_path:
    description: Workspace path where the response body is written. Use a distinct path per task since the workspace is shared.
    default: /mnt/workspace/response.json
outputs:
  status:
    description: The HTTP status code of the final response.
  body_file:
    description: Workspace path of the written response body (equals response_path).
exec:
...
| `response_path` | no | `/mnt/workspace/response.json` | Where the response body is written. |
...
| `body_file` | Workspace path of the written response body (equals `response_path`). |
